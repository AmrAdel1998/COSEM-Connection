...
class GXSettings:
    ...
    def showHelp(cls):
        ...
        print(
            " -P \t ASCII password for authentication. Use 0x prefix if hex value is used. Ex. 0x00000000."
        )
        ...
        print(
            " -d \t Used DLMS standard. Ex. -d India (DLMS, India, Italy, Saudi_Arabia, IDIS)"
        )
        ...
        for it in parameters:
            if it.tag == "r":
                ...
                if it.value.lower() == "Off".lower():
                    ...
                    raise ValueError(
                        "Invalid trace level(Off, Error, Warning, Info, Verbose)."
                    )
            elif it.tag == "p":
                #  Port.
                if not self.media:
                ...
                if it.value.lower() == "HDLC".lower():
                    ...
                    raise ValueError(
                        "Invalid interface type option."
                        + it.value
                        + " (HDLC, WRAPPER, HdlcWithModeE, Plc, PlcHdlc)"
                    )
                if (
                    modeEDefaultValues
                    and self.client.interfaceType == InterfaceType.HDLC_WITH_MODE_E
                    and isinstance(self.media, GXSerial)
                ):
                ...
                if it.value.lower() == "Suite0".lower():
                    self.client.ciphering.securitySuite = SecuritySuite.SUITE_0
                elif it.value.lower() == "Suite1".lower():
                    self.client.ciphering.securitySuite = SecuritySuite.SUITE_1
                elif it.value.lower() == "Suite2".lower():
                    self.client.ciphering.securitySuite = SecuritySuite.SUITE_2
                else:
                    raise ValueError(
                        "Invalid Ciphering option: '"
                        + it.value
                ...
                self.client.Ciphering.InvocationCounter = int(it.value)
            elif it.tag == "N":
                self.generateSecuritySetupLN = it.value
            elif it.tag == "E":
                self.exportSecuritySetupLN = it.value
            elif it.tag == "o":
                ...
                if self.client.serverAddress != 1:
                    self.client.serverAddress = GXDLMSClient.getServerAddress(
                ...
                self.client.serverAddress = GXDLMSClient.getServerAddress(
                    int(it.value), self.client.serverAddress
                )
            elif it.tag == "n":
                self.client.serverAddress = (
                    GXDLMSClient.getServerAddressFromSerialNumber(int(it.value))
                )
            elif it.tag == "W":
                self.client.gbtWindowSize = int(it.value)
            elif it.tag == "w":
                self.client.hdlcSettings.windowSizeRX = (
                    self.client.hdlcSettings.windowSizeTX
                ) = int(it.value)
            elif it.tag == "f":
                self.client.hdlcSettings.maxInfoRX = (
                    self.client.hdlcSettings.maxInfoTX
                ) = int(it.value)
            elif it.tag == "L":
